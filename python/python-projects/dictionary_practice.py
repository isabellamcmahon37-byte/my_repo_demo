def most_popular_foods(fav_foods):
    ...
    with food as the key and the list of people who like that food as the value.
    '''
    # Iterate through each person and their favorite food
    for person, food in fav_foods.items():
        if food not in foods:            # if the food isn’t already a key
            foods[food] = [person]       # create a new list with this person
        else:
            foods[food].append(person)   # add person to existing list
